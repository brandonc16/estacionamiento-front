<template>
  <v-dialog max-width="600px" v-model="dialog">
    <v-btn flat color="grey" slot="activator">
      <v-icon small left>edit</v-icon>
      <span>Editar</span>
    </v-btn>

    <v-card>
      <v-card-title>
        <h2>Editar datos de usuario</h2> 
      </v-card-title>
      <v-card-text>
        <v-form class="px-3" ref="form">
          <v-text-field label="Nombre" v-model="title" prepend-icon="person" :rules="inputRules"></v-text-field>
          <v-textarea label="Detalles" v-model="content" prepend-icon="edit" :rules="inputRules"></v-textarea>
          
          <!--
          <v-menu>
            <v-text-field :value="formattedDate" slot="activator" label="Due date" prepend-icon="date_range" :rules="inputRules"></v-text-field>
            <v-date-picker v-model="due"></v-date-picker>
          </v-menu>
          -->
          
          <v-spacer></v-spacer>

          <v-btn flat class="info mx-0 mt-3" @click="submit" :loading="loading">Guardar</v-btn>
        </v-form>
      </v-card-text>
    </v-card>
  </v-dialog>
</template>

<script>
// import format from 'date-fns/format'
// import db from '@/fb'

export default {
  data() {
    return {
      title: '',
      content: '',
      due: null,
      inputRules : [
        v => v.length >= 3 || 'Este campo es obligatorio'
      ],
      loading: false,
      dialog: false
    }
  },
  methods: {
    submit() {
      if(this.$refs.form.validate()){
        this.loading = true;

        const project = {
          title: this.title,
          content: this.content,
          // due: format(this.due, 'Do MMM YYYY'),
          due: this.due,
          person: 'The Net Ninja',
          status: 'ongoing'
        }

        console.log(project)

        /* db.collection('projects').add(project).then(() => {
          this.loading = false;
          this.dialog = false;
          this.$emit('projectAdded')
        }) */ 

        this.loading = false;
        this.dialog = false;
        this.$emit('projectAdded')
      }
    }
  },
  computed: {
    /* formattedDate() {
      return this.due ? format(this.due, 'Do MMM YYYY') : ''
    } */

  }
}
</script>
